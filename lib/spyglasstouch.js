		...
		return this.each(function(index, elem){
			$(elem).on(down, function(event){
				...
				var $this = $(event.target), //this image
					...
					newPos =	function(xPos, yPos){ //calc position of bigImage
									var newLeft = (xPos<winWidth-bigImage.width()) ? (xPos + settings.xDistance) : (xPos - settings.xDistance - bigImage.width()),
									newTop = (yPos<bigImage.height()) ? yPos + settings.yDistance : yPos - bigImage.height() - settings.yDistance ;
					...
					newImgPos =	function(xPos, yPos){ //calc position of bigImage background
									...
									return -((newLeft>0) ? (newLeft<($this.width()*settings.zoom)-bigImage.width()) ? newLeft : ($this.width()*settings.zoom)-bigImage.width() : 0) + 'px ' + 
									
									-((newTop<($this.height()*settings.zoom - bigImage.height())) ?
									 (newTop>0) ? newTop : 0 : 
									 $this.height()*settings.zoom - bigImage.height())
